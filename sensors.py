...
class DiskIO(BaseSensor):
    name = 'diskio'
    desc = _('disk io')

    def get_value(self, sensor_data):
        return self._fetch_diskio()

    def _fetch_diskio(self):
        current = [0, 0]
        # ps.disk_io_counters()
        resultcmd = os.popen('iostat -d | grep sd')
        for line in resultcmd.readlines():
            ss = list(filter(None, line.split(sep=" ")))
            r = float(ss[2])
            w = float(ss[3])
            if current[0] < r:
                current[0] = r
        ...
        # current[0] += iostat.read_time
        # current[1] += iostat.write_time
        return 'r {:>9s}/s w {:>9s}/s'.format(bytes_to_human(current[0]*1024), bytes_to_human(current[1]*1024))
